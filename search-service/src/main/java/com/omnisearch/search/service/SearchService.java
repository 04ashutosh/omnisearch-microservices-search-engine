...
public class SearchService {
        ...
        @Cacheable(value = "search-results", key = "#queryKeyword")
        public List<SearchResult> search(String queryKeyword) {
                ...
                return searchHits.getSearchHits().stream().map(hit -> {
                        ...
                        String finalSnippet = "";
                        // If Elasticsearch perfectly matched the keyword, it generates a highlighted
                        // snippet for us!
        ...
        @Cacheable(value = "suggest-results", key = "#partialPrefix")
        public List<String> getSuggestions(String partialPrefix) {
                // Use a "Match Phrase Prefix" query to find titles that start with the user's
                // keystrokes
                NativeQuery nativeQuery = NativeQuery.builder()
                ...
                // Map the heavy results into a simple list of 5 beautifully formatted String
                // Titles
